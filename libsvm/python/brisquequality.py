...
def compute_features(img):
    ...
    for itr_scale in range(scalenum):
        ...
        H,W = im.shape[:2]
        if (H != 480 and W != 720): 
            im = cv2.resize(im, (720,480), interpolation=cv2.INTER_AREA)
...
def test_measure_BRISQUE(file_path):
    # read image from given path
    dis = cv2.imread(file_path, 1)
    if(dis is None):
...
best_score_link = ""
best_score = 0
def answer(glob_path):
    minimum = 200
    for file_path in glob_path:
        path_for_imread = str(file_path)
        print(path_for_imread)
        
        qualityscore = test_measure_BRISQUE(str(file_path))
        if minimum > qualityscore:
            minimum = qualityscore
            global best_score_link
            global best_score
            best_score_link = str(file_path)
            best_score = qualityscore
        print (qualityscore)
    return best_score_link
def best_score_link_get():
    global best_score_link
    return best_score_link
def best_score_get():
    global best_score
